    ...
    >
      ...
      <swiper-item>
        <scroll-view scroll-y="ture" style="height: 100%">
          <view class="task-container" v-for="item in taskList" :key="item.id">
            <view>{{ item.label }}</view>
            <view class="list" v-for="(item2, i) in item.task" :key="i">
              <view
                class="list-left"
              ...
              <view class="list-right">
                ...
                <view class="title-small">{{ item2.desc }}</view>
              </view>
            </view>
          </view>
...
export default {
  ...
  onLoad() {
    this.dataList.forEach(item => {
      item.color = this.itemBgColor[
        Math.floor(Math.random() * this.itemBgColor.length)
      ];
    });
    uni.getSystemInfo({
      success: res => {
  ...
  methods: {
    getTaskList() {
      this.$minApi.getTaskList().then(res => {
        console.log(res);
        res.data.forEach(item => {
          item.task.forEach(item2 => {
            item2.color = this.itemBgColor[
              Math.floor(Math.random() * this.itemBgColor.length)
            ];
          });
        });
